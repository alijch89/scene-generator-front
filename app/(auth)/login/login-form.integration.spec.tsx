...
describe('LoginForm integration', () => {
  ...
  it('maps an API credential failure to the user-facing invalid state', async () => {
    ...
    jest.spyOn(global, 'fetch').mockResolvedValue(
      new Response(JSON.stringify({ code: 'INVALID_CREDENTIALS', message: 'invalid' }), {
        status: 401,
  ...
  it('sends a code and preserves the development token in the verification URL', async () => {
    const user = userEvent.setup();
    jest.spyOn(global, 'fetch').mockResolvedValue(
      new Response(JSON.stringify({ devToken: 'dev-token' }), { status: 200 }),
    );
    render(<LoginForm initialStatus="idle" />);

    await user.click(screen.getByRole('tab', { name: 'با کد یک‌بارمصرف' }));
    await user.type(screen.getByPlaceholderText('09123456789'), '09123456789');
    await user.click(screen.getByRole('button', { name: 'ارسال کد' }));

    await waitFor(() => {
      expect(router.push).toHaveBeenCalledWith(
        '/verify-phone?phone=09123456789&token=dev-token',
      );
    });
  });
});
